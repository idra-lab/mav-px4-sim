...
def generate_launch_description():
	...
	traj_replayer_node = Node(
		package='traj',
		executable='drone_r4_trajectory_replayer',
		name='drone_r4_trajectory_replayer',
		prefix='gnome-terminal --tab --',
		output='screen', 
		parameters=[{
			'trajectory_file': os.path.join(benchmark_dir, 'resource/trajectories/cube_benchmark/apace', 'benchmark.yaml'),
			# 'qos_overrides./tf_static.publisher.durability': 'transient_local',
		}],
	)
	...
	plan_after_takeoff = RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[traj_replayer_node],
            )
        )
	
	

	# add_all_actions(ld, [plan_after_takeoff])
